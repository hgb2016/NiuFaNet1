...
public class InertiaTimerTask extends TimerTask {
    ...
    public final void run() {
        ...
        if (Math.abs(mCurrentVelocityY) >= 0.0F && Math.abs(mCurrentVelocityY) <= 20F) {
            ...
            mWheelView.getHandler().sendEmptyMessage(com.contrarywind.timer.MessageHandler.WHAT_SMOOTH_SCROLL);
            return;
        }
        ...
        //刷新UI
        mWheelView.getHandler().sendEmptyMessage(com.contrarywind.timer.MessageHandler.WHAT_INVALIDATE_LOOP_VIEW);
    }
}
